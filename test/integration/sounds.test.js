...
describe('Working with sounds', () => {
    ...
    test('Check SVG image is loaded correctly', async () => {
        await loadUri(uri);
        await driver.get('http://localhost:8702/bookshelf.svg'); // Ganti dengan URL server lokal
        const svgElement = await driver.findElement(By.tagName('svg'));
    
        // Periksa apakah elemen SVG ada di halaman
